...
def test_logger_comprehensive_flow() -> None:
    ...
    with patch('sys.stdout', captured_output):
        ...
        logger.info(
            ...
            extra={"action": "login", "ip_address": "192.168.1.1", "success": True}
        )
        remove_ephemeral_info()
        ...
        logger.warning(
            "Rate limit warning",
            extra={"current_rate": 85, "max_rate": 100, "endpoint": "/api/users"}
        )
    ...
    assert len(log_entries) >= 3  # At least INFO, WARNING, ERROR (DEBUG might be filtered)
    ...
    # Verify extra_info context is present (contains both global and temporary context)
...
def test_logger_file_location_info() -> None:
    ...
    assert entry["code_context"]["function_name"] == "test_logger_file_location_info"
    assert "test_logger_comprehensive.py" in entry["code_context"]["python_file_path"]
...
def test_extra_logging_info_json_serialization_validation_init() -> None:
    ...
    with pytest.raises(ValueError, match="The provided dict is not JSON-serializable"):
...
def test_extra_logging_info_json_serialization_validation_add_more_info() -> None:
    ...
    info.add_more_info(string="value", number=123, boolean=False)  # Should not raise
    ...
    with pytest.raises(ValueError, match="The provided dict is not JSON-serializable"):
        info.add_more_info(valid_key="valid_value", invalid_key=datetime.datetime.now())
...
def test_extra_logging_info_json_serialization_edge_cases() -> None:
    ...
    test_cases = [
        ...
        ({"uuid_object": uuid.uuid4()}, "UUID objects are not JSON serializable"),
        ({"bytes_object": b"bytes"}, "Bytes are not JSON serializable"),
        ({"complex_number": 1 + 2j}, "Complex numbers are not JSON serializable"),
    ]
